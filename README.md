...
If you use [gh-aw](https://github.github.com/gh-aw/introduction/overview/) to run agents, create a companion workflow file that triggers via `workflow_run`. Pass `workflow_run_id` and the action handles everything automatically — gate logic, status normalization, trigger number resolution, and token extraction.

### Prerequisites

1. **`AGENTMETER_API_KEY` secret** — add at `github.com/<your-org>/<repo>/settings/secrets/actions`
2. **`actions: read` permission** — required for the action to call the GitHub API internally (list jobs, download artifacts). Add it to your job's `permissions` block as shown below.
...
That's the entire workflow. When `workflow_run_id` is set the action internally:
...
> **Note:** `.lock.yml` files are auto-generated by `gh aw compile`. You'll need to re-add these steps after each recompile, or open a feature request with gh-aw to include them natively.
